...
public class JenkinsUpdater implements Subject {
	...
	@Override
	public void notifyObserver(List<Observer> relevantObservers, Map <Integer, String> results) {
		List<Observer> notifiedObservers = new ArrayList<Observer>();
		for(Observer observer : relevantObservers)
		{
				String result = results.get(((CommitItem)observer).getBuildNumber());
				observer.update(_verificationStatusesMap.get(result));
